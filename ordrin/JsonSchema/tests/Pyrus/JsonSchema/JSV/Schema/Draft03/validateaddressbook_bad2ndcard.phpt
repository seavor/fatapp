--TEST--
JsonSchema: Draft 3, validate addressbook, bad second card
--FILE--
<?php
...
$a = function ($test_name, $schema_uri) use ($test, $env) {
    ...
    $test->assertSchemaValidateFail(array('The number of items is less than the required minimum [schema path: #/cards/1]'), $env->validate(array( "cards"=> array(array('foo', 'bar'), array("foo" ))), $schema),
